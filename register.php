...
if(strlen($_POST['name'])<3){
    ...
    exit();
        ...
        $prefer = mysqli_real_escape_string($db, $_POST['food']);
        $address = mysqli_real_escape_string($db, $_POST['address']);
        $age = mysqli_real_escape_string($db, $_POST['age']);
        $email = mysqli_real_escape_string($db, $_POST['email']);        
        $password = mysqli_real_escape_string($db, $_POST['password']);
        
        $query = "insert into customer(name,food,address,age,email,password) values('$name','$prefer','$address','$age' ,'$email','$password')";
        $db->query($query);
...
@-webkit-keyframes glow {
 ...
 <form action="register.php" method="post" class="form-signin" id="restaurant"><br><br>
  ...
  <input type="text"  class="form-control" placeholder="Name" name="name"  required autofocus>
     <select id="prefer" name="food" form="restaurant" class="form-control">
         <option value="">Please choose your food preference</option>
